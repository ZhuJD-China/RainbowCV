...
class Rect:
    ...
    def contains(self, thePoint):
        return thePoint.x >= self.x and thePoint.x <= (self.x + self.width) and thePoint.y >= self.y and thePoint.y <= (
                self.y + self.height)
...
class Internal:
    ...
    def trackbarXPixelToValue(self, theParams, theBounding, thePixelX):
        aRatio = float(thePixelX - (theBounding.x + self.trackbarMarginX)) / (
                theBounding.width - 2 * self.trackbarMarginX)
    ...
    def buttonI(self, theBlock, theX, theY, theIdle, theOver, theDown, theUpdateLayout):
        ...
        if aStatus == OUT:
            self._render.image(theBlock, aRect, theIdle)
        elif aStatus == OVER:
            self._render.image(theBlock, aRect, theOver)
        elif aStatus == DOWN:
            self._render.image(theBlock, aRect, theDown)
    ...
    def sparkline(self, theBlock, theValues, theX, theY, theWidth, theHeight, theColor):
        ...
        if (aHowManyValues >= 2):
            ...
            self.text(theBlock, theX, theY, 'No data.' if aHowManyValues == 0 else 'Insufficient data points.', 0.4,
                      0xCECECE, False)
...
class Render:
    ...
    def text(self, theBlock, theText, thePos, theFontScale, theColor):
        ...
        cv2.putText(theBlock.where, theText, aPosition, cv2.FONT_HERSHEY_SIMPLEX, theFontScale,
                    self._internal.hexToScalar(theColor), 1, cv2.LINE_AA)
    ...
    def counter(self, theBlock, theShape, theValue):
        ...
        aPos = Point(theShape.x + theShape.width / 2 - aTextSize.width / 2,
                     theShape.y + aTextSize.height / 2 + theShape.height / 2)
        cv2.putText(theBlock.where, theValue, (int(aPos.x), int(aPos.y)), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                    (0xCE, 0xCE, 0xCE), 1, CVUI_ANTIALISED)
    ...
    def button(self, theBlock, theState, theShape, theLabel):
        ...
        theShape.width -= 2
        theShape.height -= 2
        self.rectangle(theBlock.where, theShape, (0x42, 0x42, 0x42) if theState == OUT else (
            (0x52, 0x52, 0x52) if theState == OVER else (0x32, 0x32, 0x32)), CVUI_FILLED)
    ...
    def putText(self, theBlock, theState, theColor, theText, thePosition):
        ...
        if theText != '':
            ...
            cv2.putText(theBlock.where, theText, aPosition, cv2.FONT_HERSHEY_SIMPLEX, aFontScale, theColor, 1,
                        CVUI_ANTIALISED)
    ...
    def putTextCentered(self, theBlock, thePosition, theText):
        ...
        cv2.putText(theBlock.where, theText, (int(aPositionDecentered.x), int(aPositionDecentered.y)),
                    cv2.FONT_HERSHEY_SIMPLEX, aFontScale, (0xCE, 0xCE, 0xCE), 1, CVUI_ANTIALISED)

        return aTextSize.width

    def buttonLabel(self, theBlock, theState, theRect, theLabel, theTextSize):
        aPos = Point(theRect.x + theRect.width / 2 - theTextSize.width / 2,
        ...
        if aLabel.hasShortcut == False:
            ...
            cv2.line(theBlock.where, (int(aStart), int(aPos.y + 3)), (int(aEnd), int(aPos.y + 3)), aColor, 1,
                     CVUI_ANTIALISED)
    ...
    def trackbarPath(self, theBlock, theState, theShape, theValue, theParams, theWorkingArea):
        ...
        cv2.line(theBlock.where, (int(aRect.x + 1), int(aRect.y + aBarHeight - 2)),
                 (int(aRect.x + aRect.width - 2), int(aRect.y + aBarHeight - 2)), (0x0e, 0x0e, 0x0e))
    ...
    def trackbar(self, theBlock, theState, theShape, theValue, theParams):
        aWorkingArea = Rect(theShape.x + self._internal.trackbarMarginX, theShape.y,
                            theShape.width - 2 * self._internal.trackbarMarginX, theShape.height)
    ...
    def checkboxLabel(self, theBlock, theRect, theLabel, theTextSize, theColor):
        aPos = Point(theRect.x + theRect.width + 6,
    ...
    def window(self, theBlock, theTitleBar, theContent, theTitle):
        ...
        cv2.putText(theBlock.where, theTitle, (int(aPos.x), int(aPos.y)), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                    (0xCE, 0xCE, 0xCE), 1, CVUI_ANTIALISED)
    ...
    def sparkline(self, theBlock, theValues, theRect, theMin, theMax, theColor):
        ...
        while i <= aSize - 2:
            ...
            cv2.line(theBlock.where, (int(aPoint1.x), int(aPoint1.y)), (int(aPoint2.x), int(aPoint2.y)),
                     self._internal.hexToScalar(theColor))
            aPosX += aGap
...
def trackbar(theWhere, theX, theY, theWidth, theValue, theMin, theMax, theSegments=1, theLabelFormat='%.1Lf',
             theOptions=0, theDiscreteStep=1):
    """
...
def trackbar(theWidth, theValue, theMin, theMax, theSegments=1, theLabelFormat='%.1Lf', theOptions=0,
             theDiscreteStep=1):
    """
    Display a trackbar for numeric values that the user can increase/decrease
