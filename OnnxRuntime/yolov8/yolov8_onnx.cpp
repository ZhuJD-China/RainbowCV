...
int Yolov8Onnx::Preprocessing(const std::vector<Mat>& srcImgs, std::vector<Mat>& outSrcImgs, std::vector<Vec4d>& params)
{
	outSrcImgs.clear();
...
bool Yolov8Onnx::OnnxDetect(Mat& srcImg, std::vector<OutputSeg>& output)
{
...
bool Yolov8Onnx::OnnxBatchDetect(std::vector<Mat>& srcImgs, std::vector<std::vector<OutputSeg>>& output)
{
	vector<Vec4d> params;
