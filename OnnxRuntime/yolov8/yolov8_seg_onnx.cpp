...
int Yolov8SegOnnx::Preprocessing(const std::vector<Mat>& srcImgs, std::vector<Mat>& outSrcImgs,
...
bool Yolov8SegOnnx::OnnxDetect(Mat& srcImg, std::vector<OutputSeg>& output)
{
	std::vector<Mat> input_data = {srcImg};
...
bool Yolov8SegOnnx::OnnxBatchDetect(std::vector<Mat>& srcImgs, std::vector<std::vector<OutputSeg>>& output)
{
	vector<Vec4d> params;
	...
	vector<int> mask_protos_shape = {
		1, static_cast<int>(_outputMaskTensorShape[1]), static_cast<int>(_outputMaskTensorShape[2]),
		static_cast<int>(_outputMaskTensorShape[3])
	};
	...
	{
		Mat output0 = Mat(Size(static_cast<int>(_outputTensorShape[2]), static_cast<int>(_outputTensorShape[1])),
		                  CV_32F, all_data).t(); //[bs,116,8400]=>[bs,8400,116]
		...
		{
			//stride
			Mat scores(1, _className.size(), CV_32F, pdata + 4);
			Point classIdPoint;
