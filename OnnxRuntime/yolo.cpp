...
bool CheckParams(int netHeight, int netWidth, const int* netStride, int strideSize)
{
	...
	{
		cout << "Error:_netHeight and _netWidth must be multiple of max stride " << netStride[strideSize - 1] << "!" <<
			endl;
		return false;
	}
	return true;
...
void LetterBox(const Mat& image, Mat& outImage, Vec4d& params, const Size& newShape,
               bool autoShape, bool scaleFill, bool scaleUp, int stride, const Scalar& color)
{
	if constexpr (false)
	{
	...
	float ratio[2]{r, r};
	int new_un_pad[2] = {
		static_cast<int>(std::round((float)shape.width * r)), static_cast<int>(std::round((float)shape.height * r))
	};
	...
	if (autoShape)
	{
		dw = static_cast<float>((int)dw % stride);
		dh = static_cast<float>((int)dh % stride);
	}
	else if (scaleFill)
	{
		dw = 0.0f;
	...
	{
		resize(image, outImage, Size(new_un_pad[0], new_un_pad[1]));
	}
	else
	{
...
void GetMask(const Mat& maskProposals, const Mat& maskProtos, std::vector<OutputSeg>& output,
             const MaskParams& maskParams)
{
	...
	for (int i = 0; i < output.size(); ++i)
	{
		...
		Rect roi(static_cast<int>(params[2] / net_width * seg_width),
		         static_cast<int>(params[3] / net_height * seg_height), static_cast<int>(seg_width - params[2] / 2),
		         static_cast<int>(seg_height - params[3] / 2));
		dest = dest(roi);
